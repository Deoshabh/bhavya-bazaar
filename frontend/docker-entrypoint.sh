...
# Create runtime environment configuration (CRITICAL: Use __RUNTIME_CONFIG__ for browser compatibility)
cat > /usr/share/nginx/html/runtime-config.js << EOF
/**
 * Runtime configuration for Bhavya Bazaar (Production)
 * Generated by docker-entrypoint.sh for Coolify deployment
 * CRITICAL: NO process.env references - pure browser JavaScript only!
 */

// Define runtime configuration with environment variables from Coolify
...
console.log("✓ Runtime configuration loaded successfully from Coolify - Version 2.0.0:", window.__RUNTIME_CONFIG__);
EOF
