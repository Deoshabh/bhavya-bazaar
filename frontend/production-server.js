...
const app = express();
const PORT = process.env.PORT || 3004; // Using port 3004 to avoid conflicts with other projects
...
// API routes (if you want to proxy to backend, optional)
app.get('/api/*', (req, res) => {
  res.status(404).json({ 
    message: 'API endpoint not found on frontend server',
    suggestion: 'Use the backend API server'
  });
});
