...
Get-Process -Name "node" -ErrorAction SilentlyContinue | Format-Table Id, ProcessName, @{Name="Memory(MB)";Expression={[math]::Round($_.WorkingSet/1MB,2)}} -AutoSize
...
if ($backendStatus -and $frontendStatus) {
    ...
    Write-Host "⚠️  Some servers are not running. Please check the logs." -ForegroundColor Yellow
}

Write-Host ""
Write-Host "Server status check completed." -ForegroundColor Gray
